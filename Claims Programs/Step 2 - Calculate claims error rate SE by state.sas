* Program to run the error rate calculations ;
*DM LOG 'CLEAR';
*DM OUT 'CLEAR';
...
LIBNAME error "&mainpath./Claims Error Rates/Data";

%let pathname = &mainpath./Claims Error Rates/Results; 

%let error_type = Claim;
%include "&mainpath./Claims Error Rates/Programs/Manual Change for Claims Data Cleaning.sas" /source2;
%INCLUDE "&mainpath./Global Macros/include macros.sas";
...
libname newdir "&mainpath./Claims Error Rates/Results/Error Rate QC_Results_&sysdate.";
%let QCPath = &mainpath./Claims Error Rates/Results/Error Rate QC_Results_&sysdate.; 
%let log_folder = &QCpath.;
%let log_file_cv = Step 2 - &error_type. SE Rate Data QC &sysdate..txt;
...
*create sweight;
*read FMR data;
%PrintMessage("Read in FMR data");
%let fmr1 = /sas_data_cms/Project/PERM/Statistical Reporting/&permyear/FMR Data;


proc import datafile="&fmr1./RY&pyear. FMR stratum totals.xlsx"
					out=error.fmr_stratum_totals
                    dbms=xlsx replace;
run;

*AFR adjustment;
proc import 
datafile="&fmr1./AFR to FMR Adjustment.xls"
out=error.afr_adjustment
dbms = xls replace;
run;

*calculate sweight state;
proc import 
	datafile = "&fmr1./RY&pyear. FMR state totals sample states.xlsx"
	out = error.fmrdata
	dbms = xlsx replace;
run;
*read population totals;
%PrintMessage("Read in Population data");
data poptotals_temp;
format strata $20.;
set error.claim_pop_totals;
format program_type $2.;

*missing stratum variables;
stratum_original = stratum;
if missing(stratum) then stratum = "0";

*use type instead of program_type;
strata = COMPRESS(CAT(state_id, "&pyear.", qtr, type, stratum ));
*keep strata totclm;
run;

proc sort data=poptotals_temp;by strata;run;

data error.poptotal_duplicate;/* should be empty */
set poptotals_temp;
by strata;
...
proc sql;
	...
	from error.poptotal_duplicate; 
quit; 

%PrintMessage("Action Needed if >0: Claims data has &numobs. observations with Duplicate Strata");
%pop_totals_manual_change_nodup; /* nodupkey by state qtr type stratum totclm_clm */
...
%mend extract_relevant_pgmtype_data;


* Calculate State level estimates for Medicaid FFS ;

%macro run_rates(program_type,out,data,numerator,denominator,domainvar);

%extract_relevant_pgmtype_data(&program_type,&numerator,&denominator);
...
%mend run_rates;
%PrintMessage("Calculate State level estimates for Each Program");
/* Medicaid FFS */
%run_rates(MF,mfStatelevel_overall&type.,all,gross_error,paid2,state_strata);
%run_rates(MF,mfStatelevel_overpayments&type.,all,overpayments,paid2,state_strata);
%run_rates(MF,mfStatelevel_underpayments&type.,all,underpayments,paid2,state_strata);
%run_rates(MF,mfStatelevel_neterror&type.,all,net_error,paid2,state_strata);

/* SCHIP FFS */
%run_rates(SF,sfStatelevel_overall&type.,all,gross_error,paid2,state_strata);
%run_rates(SF,sfStatelevel_overpayments&type.,all,overpayments,paid2,state_strata);
%run_rates(SF,sfStatelevel_underpayments&type.,all,underpayments,paid2,state_strata);
%run_rates(SF,sfStatelevel_neterror&type.,all,net_error,paid2,state_strata);

/* Medicaid Managed Care */
%run_rates(MM,mmStatelevel_overall&type.,all,gross_error,paid2,state_strata);
%run_rates(MM,mmStatelevel_overpayments&type.,all,overpayments,paid2,state_strata);
%run_rates(MM,mmStatelevel_underpayments&type.,all,underpayments,paid2,state_strata);
%run_rates(MM,mmStatelevel_neterror&type.,all,net_error,paid2,state_strata);

/* SCHIP Managed Care */
%run_rates(SM,smStatelevel_overall&type.,all,gross_error,paid2,state_strata);
%run_rates(SM,smStatelevel_overpayments&type.,all,overpayments,paid2,state_strata);
%run_rates(SM,smStatelevel_underpayments&type.,all,underpayments,paid2,state_strata);
%run_rates(SM,smStatelevel_neterror&type.,all,net_error,paid2,state_strata);


/* test to make sure estimator ran correctly using results, 
	check total file */
%macro check(res_type);
data &res_type._overall;
...
%check(SM);
data projpaid;
set mf_overall mm_overall sf_overall sm_overall;
run;
proc sort; by program_type;run;

proc means data=error.total_file&type. nway noprint;
class program_type;
%if &type. = _TC %then %do;
var fmr_total_pmts;
%end; 
%else %do; 
var fmr_federal_pmts;
%end; 
output out=total_file_sum sum=;
run;

data error.total_file_results_nomatch&type.;
merge total_file_sum projpaid;
by program_type;
%if &type. = _TC %then %do;
if projpaid NE fmr_total_pmts;
%end; 
%else %do;
if projpaid NE fmr_federal_pmts;
%end;
run;

proc freq data=error.claim_merged_data&type.;
tables flagclm/list out=flagclm;run;

proc datasets library=work;
	delete qc_summary&type.Error; 
run; 

ods listing close; 
ods tagsets.ExcelXP file="&QCPath./&error_type. Error Data Problems&type..xls"
	style = Normal options(autofit_width = 'No' autofit_height = 'No')
options(embedded_titles = 'yes' embedded_footnotes='yes');
%export_QC(error.test_wt, "Sample and population data merge check - m1 ne cnt_act_sampled_clm",Error);
%export_QC(pop_merge_flag, "Sample and population data merge flag - m1 ne cnt_act_sampled_clm",Error);
%export_QC(error.in_sample_only, "Sample and population data merge check (In Sample Only) - m1 ne cnt_act_sampled_clm",Error);
%export_QC(error.in_pop_only, "Sample and population data merge check (In Sample Only) - m1 ne cnt_act_sampled_clm",Error);
%export_QC(error.weight_nomatch, "Nonmatching records between sample data and population data",Error);
%export_QC(projpaid_strata_ck, "Check any strata has $0 projpaid",Error);
%export_QC(flagclm, "non-matching records - weight and claim file",Error);
%export_QC(claim_test&type2., "Claim sweight data test - Payment values incorrect &type.",Error);
%export_QC(error.total_file_results_nomatch&type., "error.total_file_results_nomatch&type.",Error);
%export_QC(error.missing_sweight&type2., "error.missing_sweight&type2.",Error);
%export_Summary(qc_summary&type.Error, "&error_type. Error Data Problems &type. Summary");
