/******************** Macros for MR and DP cleanning  ********************/
/* 1) loopit_mr: used within import_mr module. Change toe, subqualifiers, errors from long to wide
   2) loopit_dp: used within import_dp module. Change toe, subqualifiers, errors from long to wide
   3) import_mr: import MR data and perform data cleaning
   4) import_dp: import DP data and perform data cleaning
   5) import_data: import APLUS (full_data) and merge with MR and DP data, output: perm.sample_data1&type.
   6) format_variables: reformat certain variables in sample data, output: sample_data2
   7) mergedDataCleanUp: perfrom data cleaning for merged data (APLUS - sample & Lewin data)
		indata - Lewindata, sample_data2&type.
		outdata - merged2
   8) format_for_rolling_stack(input,output): Change certain variables for stacking previous year
		input: Merged2
		output: claim_sample_data_dpmr2
*/

/* MR Errors Macro */
%macro loopit_mr;

/* Determine max number of Error Codes */
...
%mend loopit_dp;
/*create imputed FMAP Rate from ERC FMAP rate file from CMS*/
%macro ImputedFMAP; 
...
%macro import_data(outdataset, type);
...
%macro format_variables(input,dataset1,dataset2); /* variables that need to be changed */
...
ods listing;
%mend mergedDataCleanUp; 

*No longer needed; 
%macro format_for_rolling_stack(input,output);
